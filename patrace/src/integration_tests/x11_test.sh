...
# Run integration tests on X11 platform (from tests directory by default).
...
function trace() {
	echo
	echo "-- tracing $1 --"
	OUT_TRACE_FILE=testfiles/$1 LD_PRELOAD=$PATRACE_LIB/libegltrace.so INTERCEPTOR_LIB=$PATRACE_LIB/libegltrace.so TRACE_LIBEGL=$DDK_PATH/libEGL.so TRACE_LIBGLES1=$DDK_PATH/libGLESv1_CM.so TRACE_LIBGLES2=$DDK_PATH/libGLESv2.so ./$1
}
