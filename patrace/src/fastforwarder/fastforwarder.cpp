...
private:
   ...
   "#version 320 es\n\
    ...
    const GLchar *depthCopyCubeVSCode =
   "#version 320 es\n\
    in highp vec2 a_Position;\n\
    out highp vec2 v_TexCoordinate;\n\
    void main() {\n\
    ...
    const GLchar *depthCopyCubeFSCode =
   "#version 320 es\n\
    ...
    void get_depth_texture_image(GLuint sourceTexture, int width, int height, GLvoid *pixels, bool isArray, TexType texType, int id)
    {
...
// And it passed the test of GFXbench 5 Aztec whose depth texture internalFormat is GL_DEPTH_COMPONENT24.
                ...
                if (readTexFormat == GL_DEPTH_COMPONENT) {
                    get_depth_texture_image(retraceTextureId, mipmapSize.width, mipmapSize.height, texData.bufferPtr(), isArray, texType, i);
                    if (texInfo.mInternalFormat == GL_DEPTH_COMPONENT24) {
                        ...
                        DBG_LOG("WARNING: The texture of internalFormat GL_DEPTH24_STENCIL8 was never tested before. So there might be some problems!\n");
                        float *fp = (float*)texData.bufferPtr();
                        unsigned int *ip = (unsigned int *)texData.bufferPtr();
                        for (int i = 0; i < mipmapSize.width * mipmapSize.height; ++i) {
                            unsigned int factor = 0xFFFFFFFFu;
