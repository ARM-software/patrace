...
static Json::Value saveProgramUniforms(GLuint program, int count)
{
    Json::Value result = Json::arrayValue;
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            for (Json::Value &v : entry["value"])
            {
                switch (type)
                {
                case GL_SAMPLER_2D:
                case GL_SAMPLER_2D_SHADOW:
                    textures2d.insert(v.asUInt());
                    break;
                case GL_SAMPLER_CUBE:
                case GL_SAMPLER_CUBE_SHADOW:
                    textures_cubemap.insert(v.asUInt());
                    break;
                default:
                    DBG_LOG("Unhandled texture type 0x%04x (%s)\n", type, TexEnumString(type).c_str());
                    break;
                }
            }
        }
